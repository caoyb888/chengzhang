...
| defense_group → defense_recording | 1:N | 一个答辩组可有多条录像记录 |
| defense_group → defense_score | 1:N | 一个答辩组有多条评分记录 |
...
```sql
CREATE TABLE defense_recording (
  `id`              BIGINT UNSIGNED  NOT NULL AUTO_INCREMENT COMMENT '主键ID',
...
| thesis_im | 8 | im_group、im_group_member、im_message、sensitive_word、defense_group、defense_group_member、defense_recording、defense_score |
